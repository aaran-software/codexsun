...
function ShellLayout() {
  ...
  return (
    <main className="min-h-screen bg-background p-4 md:p-6">
      <div className="mx-auto flex min-h-[calc(100vh-2rem)] w-full max-w-7xl overflow-hidden rounded-[28px] border border-border/70 bg-background shadow-[0_20px_80px_rgba(15,23,42,0.08)]">
        <aside
          className={[
            'fixed inset-y-4 left-4 z-30 w-[280px] rounded-[24px] border border-border/70 bg-card p-5 shadow-xl transition-transform md:static md:translate-x-0 md:rounded-none md:border-0 md:border-r md:shadow-none',
        ...
        >
          <div className="flex items-start justify-between gap-4">
            <div className="space-y-2">
              ...
              <p className="text-sm leading-6 text-muted-foreground">
                Apps mount through the shell, but app-owned behavior stays outside
                the host.
              </p>
            </div>
            <Button
          ...
          <nav className="mt-8 space-y-2">
            {visibleModules.map((moduleDefinition) => (
              ...
              >
                <p className="text-sm font-semibold">{moduleDefinition.navLabel}</p>
                <p className="mt-1 text-sm leading-6">{moduleDefinition.summary}</p>
              </NavLink>
            ))}
          </nav>
        </aside>
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          <header className="border-b border-border/70 px-5 py-4 md:px-8">
