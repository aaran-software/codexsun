...
export default function UserPage() {
    const { authFetch } = useAuth(); // ✅ use authFetch instead of fetch
    ...
    const fetchUsers = async () => {
        try {
            const res = await authFetch("http://localhost:3000/users");
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            const res = await authFetch(`http://localhost:3000/users/${id}`, {
                method: "DELETE",
            });
    ...
    const handleUpdateUser = async (e: React.FormEvent) => {
        ...
        try {
            const res = await authFetch(`http://localhost:3000/users/${editingUser.id}`, {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ name, email, password }),
            });
