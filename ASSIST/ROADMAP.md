...
- Go-live hardening phase: replace authenticated storefront browsing dependencies with a public catalog API surface, then complete server-backed wishlist or review flows, checkout hardening, payment reconciliation, stock reservation, and production-readiness work for public launch.
- Payment readiness phase: add a single-provider production payment path first, with Razorpay checkout, UPI-capable collection, webhook verification, and order-payment reconciliation before expanding to any additional gateways.
