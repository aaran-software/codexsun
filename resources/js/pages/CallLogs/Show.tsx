...
// MAIN COMPONENT
// ---------------------------------------------------------------------
export default function CallLogNotes({ callLogId }: Props) {
    ...
    // Filter notes recursively based on search term
    const filterNotes = (notes: Note[], term: string): Note[] => {
        if (!term) return notes;
        const lowerTerm = term.toLowerCase();
        return notes
            .filter((note) => {
                const matchesNote =
                    note.note.toLowerCase().includes(lowerTerm) ||
                    (note.user?.name.toLowerCase().includes(lowerTerm) ?? false);
                const matchingReplies = note.replies ? filterNotes(note.replies, term) : [];
                return matchesNote || matchingReplies.length > 0;
            })
            .map((note) => ({
                ...note,
                replies: note.replies ? filterNotes(note.replies, term) : undefined,
            }));
    };
    ...
    const submitNote = (e: React.FormEvent) => {
        ...
        post(route('calls.notes.store', callLogId), {
            preserveScroll: true,
            onSuccess: () => {
                reset();
                setReplyingTo(null);
    ...
    const saveEdit = (noteId: number) => {
        put(route('calls.notes.update', [callLogId, noteId]), {
            preserveScroll: true,
            onSuccess: () => {
                setEditing(null);
                handleReload();
    ...
    const deleteNote = (noteId: number) => {
        ...
        router.delete(
            route('calls.notes.destroy', [callLogId, noteId]),
            {
                preserveScroll: true,
        ...
        return (
            ...
            >
                ...
                >
                    ...
                    {/* Actions */}
                    <div className="mt-2 flex flex-col gap-1 opacity-0 transition-opacity group-hover:opacity-100">
                        <TooltipProvider>
                            <Tooltip>
        ...
        <div className="mt-6 flex flex-col rounded-xl border bg-card p-4 shadow-md">
            ...
            <ScrollArea className="flex-1 pr-3" ref={scrollRef}>
                {notes.length === 0 && !searchTerm ? (
                    ...
                    <div className="space-y-4">{filteredNotes.map(renderNote)}</div>
                )}
            </ScrollArea>
