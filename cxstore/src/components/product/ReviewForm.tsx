...
export function ReviewForm({
  ...
  return (
    <form onSubmit={handleSubmit} className="grid gap-4 rounded-[1.8rem] border border-border/60 bg-card p-5">
      ...
      <div className="space-y-2">
        <Label>Review</Label>
        <Textarea value={review} onChange={(event) => setReview(event.target.value)} rows={4} placeholder="Share your buying experience" />
      </div>
      <div className="flex justify-end">
        <Button type="submit" className="rounded-full px-5">Submit Review</Button>
      </div>
    </form>
  )
}
