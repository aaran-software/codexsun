import { useEffect, useState, useCallback, useMemo } from "react"
import { SparklesIcon, ArrowRightIcon, ChevronLeftIcon, ChevronRightIcon } from "lucide-react"
import { Link } from "react-router-dom"
import useEmblaCarousel from "embla-carousel-react"

import { Button, buttonVariants } from "@/components/ui/button"
import { cn } from "@/lib/utils"

const dealSlides = [
  {
    id: 1,
    badge: "Limited Time",
    title: "Seasonal Style Refresh: Up to 40% Off",
    description: "Discover fresh looks for the new season with exclusive multi-vendor discounts. Shop trending fashion and accessories.",
    buttonText: "Shop the Sale",
    href: "/search?category=fashion&sort=discount-desc",
  },
  {
    id: 2,
    badge: "Tech Upgrade",
    title: "Unbeatable Deals on Smart Gadgets",
    description: "Upgrade your smart home and daily carry with top-rated electronics. Wholesale tiers unlocked for all customers this weekend.",
    buttonText: "Browse Electronics",
    href: "/search?category=electronics",
  },
  {
    id: 3,
    badge: "Clearance",
    title: "Massive Clearance on Home Essentials",
    description: "Transform your living space without breaking the bank. Fast commerce flows guarantee delivery by the weekend.",
    buttonText: "View Clearance",
    href: "/search?category=home-decor",
  },
]

// Predefined set of beautiful, soft pastel/warm gradient backgrounds for the deal banners
const backgroundGradients = [
  "linear-gradient(135deg, #fdfbfb 0%, #ebedee 100%)",
  "linear-gradient(135deg, #e0c3fc 0%, #8ec5fc 100%)",
  "linear-gradient(135deg, #f6d365 0%, #fda085 100%)",
  "linear-gradient(135deg, #84fab0 0%, #8fd3f4 100%)",
  "linear-gradient(135deg, #fbc2eb 0%, #a6c1ee 100%)",
  "linear-gradient(135deg, #ffecd2 0%, #fcb69f 100%)",
  "linear-gradient(135deg, #cfd9df 0%, #e2ebf0 100%)",
]

export function DealBanner() {
  const [emblaRef, emblaApi] = useEmblaCarousel({ loop: true, align: "start" })
  const [selectedIndex, setSelectedIndex] = useState(0)

  // Assign a random background gradient to each slide exactly once on mount
  const slideBackgrounds = useMemo(() => {
    return dealSlides.map(() => {
      const randomIndex = Math.floor(Math.random() * backgroundGradients.length)
      return backgroundGradients[randomIndex]
    })
  }, [])

  const scrollPrev = useCallback(() => {
    if (emblaApi) emblaApi.scrollPrev()
  }, [emblaApi])

  const scrollNext = useCallback(() => {
    if (emblaApi) emblaApi.scrollNext()
  }, [emblaApi])

  const onSelect = useCallback(() => {
    if (!emblaApi) return
    setSelectedIndex(emblaApi.selectedScrollSnap())
  }, [emblaApi, setSelectedIndex])

  useEffect(() => {
    if (!emblaApi) return
    
    // Auto-advance
    const autoPlayInterval = setInterval(() => {
      emblaApi.scrollNext()
    }, 5000)

    onSelect()
    emblaApi.on("select", onSelect)
    
    return () => {
      clearInterval(autoPlayInterval)
      emblaApi.off("select", onSelect)
    }
  }, [emblaApi, onSelect])

  return (
    <section className="relative overflow-hidden rounded-[2rem] border border-border/60 shadow-[0_22px_60px_-38px_rgba(86,52,18,0.25)] transition-all duration-700 ease-in-out">
      <div className="overflow-hidden" ref={emblaRef}>
        <div className="flex touch-pan-y">
          {dealSlides.map((slide, index) => (
            <div key={slide.id} className="min-w-0 flex-[0_0_100%]">
              <div 
                className="flex flex-col gap-6 p-6 sm:p-8 md:p-10 lg:flex-row lg:items-center lg:justify-between transition-all duration-1000"
                style={{ background: slideBackgrounds[index] }}
              >
                <div className="space-y-3 lg:max-w-[70%]">
                  <div className="inline-flex items-center gap-2 rounded-full bg-black/5 px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.2em] text-black/80 backdrop-blur-sm sm:text-xs">
                    <SparklesIcon className="size-3.5 sm:size-4" />
                    {slide.badge}
                  </div>
                  <h2 className="text-2xl font-bold tracking-tight text-black sm:text-3xl md:text-4xl">
                    {slide.title}
                  </h2>
                  <p className="max-w-2xl text-sm leading-relaxed text-black/70 sm:text-base">
                    {slide.description}
                  </p>
                </div>
                
                <div className="flex shrink-0 items-center justify-between gap-4 lg:flex-col lg:items-end">
                  <Link 
                    to={slide.href} 
                    className={buttonVariants({ className: "group h-11 rounded-full bg-black px-6 text-sm font-semibold text-white shadow-md hover:bg-black/80 sm:h-12 sm:px-8 sm:text-base" })}
                  >
                    {slide.buttonText}
                    <ArrowRightIcon className="ml-2 size-4 transition-transform group-hover:translate-x-1" />
                  </Link>
                  
                  {/* Inline controls for larger screens */}
                  <div className="hidden items-center gap-2 lg:flex">
                    <Button
                      variant="outline"
                      size="icon"
                      onClick={scrollPrev}
                      className="size-10 rounded-full border-black/10 bg-white/40 text-black backdrop-blur-md hover:bg-white/60 hover:text-black"
                    >
                      <ChevronLeftIcon className="size-4" />
                    </Button>
                    <Button
                      variant="outline"
                      size="icon"
                      onClick={scrollNext}
                      className="size-10 rounded-full border-black/10 bg-white/40 text-black backdrop-blur-md hover:bg-white/60 hover:text-black"
                    >
                      <ChevronRightIcon className="size-4" />
                    </Button>
                  </div>
                </div>
              </div>
            </div>
          ))}
        </div>
      </div>
      
      {/* Mobile/Tablet dots indicator */}
      <div className="absolute bottom-4 left-0 right-0 flex justify-center gap-1.5 lg:hidden">
        {dealSlides.map((_, index) => (
          <div
            key={index}
            className={cn(
              "h-1.5 rounded-full transition-all duration-300",
              index === selectedIndex ? "w-4 bg-black/60" : "w-1.5 bg-black/20"
            )}
          />
        ))}
      </div>
    </section>
  )
}
