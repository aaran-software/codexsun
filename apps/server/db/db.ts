...
export async function query<T = any>(
    ...
    try {
      ...
      // In query() or pooledQuery
      // const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('Query Timeout')), 5000));  // 5s timeout
      // const result = await Promise.race([pooledQuery(sql, normalized), timeoutPromise]);
