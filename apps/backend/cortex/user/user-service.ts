...
export async function createUserService(user: Omit<User, 'id' | 'created_at' | 'updated_at' | 'password_hash'> & { password: string, tenant_id: string }): Promise<User> {
    ...
    const fullUser: Omit<User, 'id' | 'created_at' | 'updated_at'> = {
...
export async function updateUserService(id: number, updates: Partial<Omit<User, 'id' | 'created_at' | 'updated_at'>> & { password?: string }, tenantId: string): Promise<User | null> {
    if (updates.password) {
