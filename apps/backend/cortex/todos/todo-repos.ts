...
export async function createUser(todo: Todo): Promise<QueryResult<Todo>> {
    const { todoname, email, password_hash, mobile, status, role_id, email_verified } = todo;
    return query<Todo>(
        'INSERT INTO todos (todoname, email, password_hash, mobile, status, role_id, email_verified) VALUES (?, ?, ?, ?, ?, ?, ?)',
        [todoname, email, password_hash, mobile, status, role_id, email_verified]
    );
}
