...
export async function server(): Promise<void> {
    ...
    const server: Server = createServer((req, res) => {
        try {
            router.routeRequest(req, res).catch((err: Error) => {
                logger.error("Error handling request", {error: err.message, url: req.url, method: req.method});
    ...
    const shutdown = async () => {
        try {
            ...
            logger.error("Error during shutdown process", {error: (err instanceof Error) ? err.message : String(err)});
            process.exit(1);
        }
    };
    ...
    process.on("SIGINT", async () => {
        try {
            ...
            logger.error("Error handling SIGINT", {error: (err instanceof Error) ? err.message : String(err)});
            process.exit(1);
        }
    });
    ...
    process.on("SIGTERM", async () => {
        try {
            ...
            logger.error("Error handling SIGTERM", {error: (err instanceof Error) ? err.message : String(err)});
            process.exit(1);
        }
    });
    ...
    process.on("unhandledRejection", (reason, promise) => {
        logger.error("Unhandled promise rejection", {error: String(reason), promise: String(promise)});
    });
}
