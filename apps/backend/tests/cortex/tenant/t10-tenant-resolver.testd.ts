...
describe("[1.] resolveTenant", () => {
    ...
    it("[test 1] throws on missing email", async () => {
        await expect(resolveByEmail({ body: { email: "", password: "pass" } })).rejects.toThrow("Valid email is required");
    });

    it("[test 2] throws if no user found", async () => {
        (query as jest.Mock).mockResolvedValueOnce({ rows: [] });
        await expect(resolveByEmail({ body: { email: "test@email.com", password: "pass" } })).rejects.toThrow("No tenant associated");
    });

    it("[test 3] throws if multiple users found", async () => {
        (query as jest.Mock).mockResolvedValueOnce({ rows: [{ tenant_id: "1" }, { tenant_id: "2" }] });
        await expect(resolveByEmail({ body: { email: "test@email.com", password: "pass" } })).rejects.toThrow("Multiple tenants found");
    });
    ...
    it("[test 4] throws if no tenant found", async () => {
        ...
        await expect(resolveByEmail({ body: { email: "test@email.com", password: "pass" } })).rejects.toThrow("Tenant not found");
    });
    ...
    it("[test 5] throws on incomplete tenant config", async () => {
        ...
        await expect(resolveByEmail({ body: { email: "test@email.com", password: "pass" } })).rejects.toThrow("Incomplete tenant configuration");
    });
    ...
    it("[test 8] handles resolution error", async () => {
        ...
        await expect(resolveByEmail({ body: { email: "test@email.com", password: "pass" } })).rejects.toThrow("Tenant resolution failed for email test@email.com: query fail");
    });
