...
export class DutyRepository {
    ...
    static async getDutyById(tenantId: string, id: number, userId: string): Promise<Duty | null> {
        const result = await query<Duty>(
    ...
    static async createDuty(tenantId: string, task: Duty): Promise<Duty> {
        return await withTransaction(async (client) => {
            ...
            const selectResult = await client.query<Duty>(
                `SELECT * FROM tasks WHERE id = ? AND (created_by = ? OR assigned_to = ?)`,
                [insertId, task.created_by, task.assigned_to],
                tenantId
            );

            const newDuty = selectResult.rows[0];
            if (!newDuty) {
    ...
    static async updateDuty(tenantId: string, id: number, task: Partial<Duty>, userId: string): Promise<Duty | null> {
    ...
    static async deleteDuty(tenantId: string, id: number, userId: string): Promise<boolean> {
        return await withTransaction(async (client) => {
