...
const SortableTodoItem: React.FC<{
  ...
  return (
    <li
      ...
      style={style}
      className={cn(
        'flex items-center p-4 bg-white rounded-md shadow-md mb-2',
        todo.completed && 'opacity-75',
        isDragging && 'opacity-50',
        isOver && 'ring-2 ring-blue-500'
      )}
    >
      <GripVertical className="w-5 h-5 text-gray-400 mr-2 cursor-grab" {...attributes} {...listeners} />
  ...
  const addTodo = async () => {
    ...
    try {
      const todoData = {
        ...
        completed: false,
        category,
        due_date: formatDateForMySQL(dueDate), // Format date for MySQL
        priority,
      };
  ...
  const saveEdit = async (id: number) => {
    ...
    try {
      const updates = {
        ...
        due_date: formatDateForMySQL(interactionState.editDueDate), // Format date for MySQL
